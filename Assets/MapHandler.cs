...
public class MapHandler : MonoBehaviour
{
	...
	public (int, Color) getClickedCountry() {
		(int xPixelUnity, int yPixelUnity) = getPixelClicked();
		...
		return (idx, pixelColor);
	}

	// Returns which pixel on the map was clicked in unity's reference
	public (int, int) getPixelClicked() {
		int pixelWidth = 5632;
		...
		return (xPixelUnity, yPixelUnity);
	}

	public void paintCountry(int indexOfCountry, Color pixelColor) {
		//print("Painting " + countries[indexOfCountry]);
		int[] pixels = countryPixels[indexOfCountry];
